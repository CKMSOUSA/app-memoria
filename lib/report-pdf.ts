...
export function exportUserReportPdf(input: {
  ...
  const printWindow = createPrintWindow(`Relatório-${input.usuario.nome}`);
  ...
  writeSection(
    printWindow,
    `<h2>Próxima atividade sugerida</h2>
...
export function exportAdminReportPdf(input: {
  ...
  writeSection(
    ...
    `<h2>Resumo geral</h2>
     <div class="grid">
       <div class="card"><strong>Usuários monitorados</strong><div class="line">${input.users.length}</div></div>
       ...
       <div class="card"><strong>Sessões registradas</strong><div class="line">${input.users.reduce((sum, item) => sum + item.history.length, 0)}</div></div>
     </div>`,
  );
     ...
     ${input.users
       .map(
         ({ user, history }) => `<div class="card">
           ...
           <p class="line">Sessões: ${history.length}</p>
           <p class="line">Última atividade: ${history[0]?.playedAt ? new Date(history[0].playedAt).toLocaleDateString("pt-BR") : "Sem atividade"}</p>
         </div>`,
       )
       .join("")}`,
  );
